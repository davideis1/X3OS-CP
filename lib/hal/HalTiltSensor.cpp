...
void HalTiltSensor::update(const uint8_t mode, const uint8_t orientation, const bool inReader) {
  ...
  if ((mode != TinyRdrTiltPageTurn::TILT_OFF) && !_isAwake) {
    _isAwake = wake();
    return;
  } else if ((mode == TinyRdrTiltPageTurn::TILT_OFF) && _isAwake) {
    _isAwake = !deepSleep();
    return;
  }

  // If disabled, skip the rest of the polling logic and avoid unnecessary I2C traffic in non-reader activities
  if ((mode == TinyRdrTiltPageTurn::TILT_OFF) || !inReader) {
    return;
  }
  ...
  float tiltAxis;
  switch (orientation) {
    case TinyRdrOrientation::PORTRAIT:
      tiltAxis = mode == TinyRdrTiltPageTurn::TILT_INVERTED ? -gx : gx;
      break;
    case TinyRdrOrientation::INVERTED:
      tiltAxis = mode == TinyRdrTiltPageTurn::TILT_INVERTED ? gx : -gx;
      break;
    case TinyRdrOrientation::LANDSCAPE_CW:
      tiltAxis = mode == TinyRdrTiltPageTurn::TILT_INVERTED ? gy : -gy;
      break;
    case TinyRdrOrientation::LANDSCAPE_CCW:
      tiltAxis = mode == TinyRdrTiltPageTurn::TILT_INVERTED ? -gy : gy;
      break;
    default:
      tiltAxis = gx;
