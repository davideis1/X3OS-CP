...
/**
 * TinyRdr position representation.
 */
struct TinyRdrPosition {
...
/**
 * Maps between TinyRdr and SavedProgress position formats, such as those used by KOReader.
 *
 * TinyRdr tracks position as (spineIndex, pageNumber).
 * SavedProgress uses XPath-like strings + percentage.
 *
 * Since TinyRdr discards HTML structure during parsing, we generate
...
class ProgressMapper {
 public:
  /**
   * Convert TinyRdr position to SavedProgress format.
   *
   * @param epub The EPUB book
   * @param pos TinyRdr position
   * @return SavedProgress position
   */
  static SavedProgressPosition toSavedProgress(const std::shared_ptr<Epub>& epub, const TinyRdrPosition& pos);

  /**
   * Convert SavedProgress position to TinyRdr format.
   *
   ...
   * @return TinyRdr position
   */
  static TinyRdrPosition toTinyRdr(const std::shared_ptr<Epub>& epub, const SavedProgressPosition& savedPos,
