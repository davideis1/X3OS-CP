  ...
  auto renderLines = [&]() {
    ...
    for (const auto& line : currentPageLines) {
      if (!line.empty()) {
        ...
        if (lineIsRtl && (effectiveAlignment == TinyRdrSettings::LEFT_ALIGN ||
                          effectiveAlignment == TinyRdrSettings::JUSTIFIED)) {
          effectiveAlignment = TinyRdrSettings::RIGHT_ALIGN;
        }
        const int textWidth = renderer.getTextAdvanceX(cachedFontId, line.c_str(), EpdFontFamily::REGULAR);

        // Apply text alignment
        switch (effectiveAlignment) {
          case TinyRdrSettings::LEFT_ALIGN:
          default:
            // x already set to left margin
            break;
          case TinyRdrSettings::CENTER_ALIGN: {
            x = cachedOrientedMarginLeft + (contentWidth - textWidth) / 2;
            break;
          }
          case TinyRdrSettings::RIGHT_ALIGN: {
            x = cachedOrientedMarginLeft + contentWidth - textWidth;
            break;
          }
          case TinyRdrSettings::JUSTIFIED:
...
void TxtReaderActivity::renderStatusBar() const {
  ...
  std::string title;
  if (SETTINGS.statusBarTitle != TinyRdrSettings::STATUS_BAR_TITLE::HIDE_TITLE) {
    title = txt->getTitle();
  }
