...
void applyLegacyStatusBarSettings(TinyRdrSettings& settings) {
  switch (static_cast<TinyRdrSettings::STATUS_BAR_MODE>(settings.statusBar)) {
    case TinyRdrSettings::NONE:
      settings.statusBarChapterPageCount = 0;
      settings.statusBarBookProgressPercentage = 0;
      settings.statusBarProgressBar = TinyRdrSettings::HIDE_PROGRESS;
      settings.statusBarTitle = TinyRdrSettings::HIDE_TITLE;
      settings.statusBarBattery = 0;
      break;
    case TinyRdrSettings::NO_PROGRESS:
      settings.statusBarChapterPageCount = 0;
      settings.statusBarBookProgressPercentage = 0;
      settings.statusBarProgressBar = TinyRdrSettings::HIDE_PROGRESS;
      settings.statusBarTitle = TinyRdrSettings::CHAPTER_TITLE;
      settings.statusBarBattery = 1;
      break;
    case TinyRdrSettings::BOOK_PROGRESS_BAR:
      settings.statusBarChapterPageCount = 1;
      settings.statusBarBookProgressPercentage = 0;
      settings.statusBarProgressBar = TinyRdrSettings::BOOK_PROGRESS;
      settings.statusBarTitle = TinyRdrSettings::CHAPTER_TITLE;
      settings.statusBarBattery = 1;
      break;
    case TinyRdrSettings::ONLY_BOOK_PROGRESS_BAR:
      settings.statusBarChapterPageCount = 1;
      settings.statusBarBookProgressPercentage = 0;
      settings.statusBarProgressBar = TinyRdrSettings::BOOK_PROGRESS;
      settings.statusBarTitle = TinyRdrSettings::HIDE_TITLE;
      settings.statusBarBattery = 0;
      break;
    case TinyRdrSettings::CHAPTER_PROGRESS_BAR:
      settings.statusBarChapterPageCount = 0;
      settings.statusBarBookProgressPercentage = 1;
      settings.statusBarProgressBar = TinyRdrSettings::CHAPTER_PROGRESS;
      settings.statusBarTitle = TinyRdrSettings::CHAPTER_TITLE;
      settings.statusBarBattery = 1;
      break;
    case TinyRdrSettings::FULL:
    default:
      settings.statusBarChapterPageCount = 1;
      settings.statusBarBookProgressPercentage = 1;
      settings.statusBarProgressBar = TinyRdrSettings::HIDE_PROGRESS;
...
bool JsonSettingsIO::saveState(const TinyRdrState& s, const char* path) {
  JsonDocument doc;
  doc["openEpubPath"] = s.openEpubPath;
  JsonArray recentArr = doc["recentSleepImages"].to<JsonArray>();
  for (int i = 0; i < TinyRdrState::SLEEP_RECENT_COUNT; i++) recentArr.add(s.recentSleepImages[i]);
...
bool JsonSettingsIO::loadState(TinyRdrState& s, const char* json) {
  JsonDocument doc;
  ...
  const int actualCount = recentArr.isNull() ? 0
                                             : std::min(static_cast<int>(recentArr.size()),
                                                        static_cast<int>(TinyRdrState::SLEEP_RECENT_COUNT));
  ...
  if (s.recentSleepPos >= TinyRdrState::SLEEP_RECENT_COUNT)
    s.recentSleepPos = actualCount > 0 ? s.recentSleepPos % TinyRdrState::SLEEP_RECENT_COUNT : 0;
...
bool JsonSettingsIO::saveSettings(const TinyRdrSettings& s, const char* path) {
  JsonDocument doc;
...
bool JsonSettingsIO::loadSettings(TinyRdrSettings& s, const char* json, bool* needsResave) {
  ...
  if (doc["sleepTimeoutMinutes"].isNull() && !doc["sleepTimeout"].isNull()) {
    const uint8_t legacyValue =
        clamp(doc["sleepTimeout"] | (uint8_t)TinyRdrSettings::SLEEP_10_MIN, TinyRdrSettings::SLEEP_TIMEOUT_COUNT,
              (uint8_t)TinyRdrSettings::SLEEP_10_MIN);
    s.sleepTimeoutMinutes = TinyRdrSettings::sleepTimeoutEnumToMinutes(legacyValue);
    if (needsResave) *needsResave = true;
  }
  // Front button remap — managed by RemapFrontButtons sub-activity, not in SettingsList.
  using S = TinyRdrSettings;
  s.frontButtonBack =
  ...
  TinyRdrSettings::validateFrontButtonMapping(s);

  // Font family — uses dynamic getter/setter in SettingsList so the generic loop skips it.
  const uint8_t storedFontFamily = doc["fontFamily"] | (uint8_t)0;
  s.fontFamily = clamp(storedFontFamily, TinyRdrSettings::BUILTIN_FONT_COUNT, 0);
  // SD card font family name — not in SettingsList, load manually
  const char* sfn = doc["sdFontFamilyName"] | "";
  strncpy(s.sdFontFamilyName, sfn, sizeof(s.sdFontFamilyName) - 1);
  s.sdFontFamilyName[sizeof(s.sdFontFamilyName) - 1] = '\0';
  if (storedFontFamily == TinyRdrSettings::LEGACY_OPENDYSLEXIC && s.sdFontFamilyName[0] == '\0') {
    s.fontFamily = TinyRdrSettings::NOTOSERIF;
    strncpy(s.sdFontFamilyName, "OpenDyslexic", sizeof(s.sdFontFamilyName) - 1);
    s.sdFontFamilyName[sizeof(s.sdFontFamilyName) - 1] = '\0';
    if (needsResave) *needsResave = true;
  } else if (storedFontFamily >= TinyRdrSettings::BUILTIN_FONT_COUNT) {
    if (needsResave) *needsResave = true;
  }
