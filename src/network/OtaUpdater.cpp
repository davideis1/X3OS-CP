...
OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  LOG_DBG("OTA", "Checking for update (current: %s)", TINYRDR_VERSION);
...
bool OtaUpdater::isUpdateNewer() const {
  if (!updateAvailable || latestVersion.empty() || latestVersion == TINYRDR_VERSION) {
    return false;
  }
  ...
  const auto currentVersion = TINYRDR_VERSION;
