...
TinyRdr runs on real hardware, so debugging usually combines local build checks and on-device logs.
...
- Whether issue reproduces after clearing `.tinyrdr/` cache on SD card
