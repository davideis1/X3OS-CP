...
TinyRdr is firmware for the Xteink X4 (unaffiliated with Xteink), built with PlatformIO targeting the ESP32-C3 microcontroller.
...
- `src/TinyRdrSettings.h` (`SETTINGS`): user preferences and behavior flags
- `src/TinyRdrState.h` (`APP_STATE`): runtime/session state such as current book and sleep context
...
Network file transfer is controlled by `src/activities/network/TinyRdrWebServerActivity.h` and served by `src/network/TinyRdrWebServer.h`.
