...
available while TinyRdr is in File Transfer or Calibre Wireless mode.
...
Examples use `tinyrdr.local`. If mDNS does not resolve on your network, use
...
```bash
curl http://tinyrdr.local/api/status
```
...
```bash
curl "http://tinyrdr.local/api/files?path=/Books"
```
...
```bash
curl -OJ "http://tinyrdr.local/download?path=/Books/MyBook.epub"
```
...
```bash
curl -X POST -F "file=@mybook.epub" "http://tinyrdr.local/upload?path=/Books"
```
...
```bash
curl -X POST -d "name=NewFolder&path=/" http://tinyrdr.local/mkdir
```
...
```bash
curl -X POST -d "path=/Books/old.epub&name=new.epub" http://tinyrdr.local/rename
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub&dest=/Read" http://tinyrdr.local/move
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub" http://tinyrdr.local/delete
curl -X POST -d 'paths=["/Books/old.epub","/OldFolder"]' http://tinyrdr.local/delete
```
...
```bash
curl http://tinyrdr.local/api/settings
```
...
curl -X POST \
  ...
  http://tinyrdr.local/api/settings
```
...
```bash
curl http://tinyrdr.local/api/fonts
```
...
curl -X POST \
  ...
  http://tinyrdr.local/api/fonts/upload
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"family":"Literata"}' \
  http://tinyrdr.local/api/fonts/delete
```
...
```bash
curl http://tinyrdr.local/api/opds
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://tinyrdr.local/api/opds/delete
```
...
```bash
curl http://tinyrdr.local/api/wifi
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://tinyrdr.local/api/wifi/delete
```
...
- `tinyrdr.local` is advertised with mDNS when available.
- `/api/status` returns `"mode": "STA"` and RSSI in dBm.

### Access Point Mode (AP)

- Device creates an open hotspot named `TinyRdr`.
