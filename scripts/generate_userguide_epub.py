...
OUTPUT_EPUB = ROOT / "TinyRdr_User_Guide.epub"
...
def split_chapters(html: str):
    ...
    if not positions:
        return [('intro', 'TinyRdr User Guide', html)]
...
def build_epub():
    ...
    book = epub.EpubBook()
    book.set_identifier('tinyrdr-user-guide-v1')
    book.set_title('TinyRdr User Guide')
    book.set_language('en')
    book.add_author('TinyRdr Project')
    ...
    # Full-size cover image (480×800) — used by TinyRdr for home-screen thumbnail.
    ...
    # EPUB 2 cover declaration — TinyRdr's Tier 1 OPF lookup
    book.add_metadata('OPF', 'meta', '', {'name': 'cover', 'content': 'cover-image'})

    # Cover page XHTML — first readable page in the spine
    cover_page = epub.EpubHtml(title='TinyRdr User Guide', file_name='cover.xhtml', lang='en')
    cover_page.content = make_xhtml(
        'TinyRdr User Guide',
        '<div class="cover-page">'
        '<img src="images/cover.png" alt="TinyRdr User Guide cover"/>'
        '</div>',
    )
    ...
    # nav is excluded from the spine — TinyRdr locates it via properties="nav" in
